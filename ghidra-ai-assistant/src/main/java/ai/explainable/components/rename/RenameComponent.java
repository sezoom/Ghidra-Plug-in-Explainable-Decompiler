...
public class RenameComponent implements AnalysisComponent<RenameResult> {
    ...
    @Override
    public RenameResult analyze(AnalysisContext context, BackendClient client) throws Exception {
    ...
    @Override
    public void renderResult(AnalysisContext context, RenameResult result, AnalysisView view) {
        RenamePreviewBuilder.PreviewData preview = RenamePreviewBuilder.build(context, result);
    ...
    public boolean supportsApply(AnalysisContext context, RenameResult result) {
        return RenamePreviewBuilder.getApplicableFunctionRename(context, result) != null ||
            !RenamePreviewBuilder.getApplicableVariableRenames(context, result).isEmpty();
    }

    @Override
    public void apply(AnalysisContext context, RenameResult result, AnalysisView view) throws Exception {
        RenameItem functionRename = RenamePreviewBuilder.getApplicableFunctionRename(context, result);
        List<RenameItem> variableRenames = RenamePreviewBuilder.getApplicableVariableRenames(context, result);
        if (context.getProgram() == null || context.getFunction() == null ||
                (functionRename == null && variableRenames.isEmpty())) {
            view.showInfo("AI Rename", "Load a rename suggestion before applying it.");
            return;
        }

        if (!view.confirm("Apply AI Renames?", buildApplyConfirmationMessage(functionRename, variableRenames))) {
            return;
        }
        ...
        try {
            if (functionRename != null) {
                context.getFunction().setName(functionRename.getNewName(), SourceType.USER_DEFINED);
                appliedCount++;
            }

            for (RenameItem item : variableRenames) {
                AnalysisContext.VariableTarget target = context.getVariableTargetsById().get(item.getTargetId());
                if (target == null) {
                    failures.add(item.getOldName() + " -> " + item.getNewName() + " (missing target)");
                    continue;
                }
                try {
                    HighFunctionDBUtil.updateDBVariable(target.getHighSymbol(), item.getNewName(), null,
                        SourceType.USER_DEFINED);
                    appliedCount++;
                }
                catch (DuplicateNameException | InvalidInputException | UnsupportedOperationException ex) {
                    failures.add(item.getOldName() + " -> " + item.getNewName() + " (" + ex.getMessage() + ")");
                }
            }
        ...
        view.showResultText(buildApplySummary(appliedCount, functionRename, variableRenames, failures));
        if (appliedCount > 0) {
            view.showInfo("AI Rename", "Applied " + appliedCount + " rename suggestion(s). Reload the decompilation to refresh context.");
        }
    }

    private List<VariableCandidate> buildVariableCandidates(AnalysisContext context) {
        if (context.getVariableTargetsById().isEmpty()) {
            return Collections.emptyList();
        }
        List<VariableCandidate> candidates = new ArrayList<>();
        for (AnalysisContext.VariableTarget target : context.getVariableTargetsById().values()) {
            candidates.add(new VariableCandidate(
                target.getTargetId(),
                target.getKind(),
    ...
    private String formatRenameResult(AnalysisContext context, RenameResult result) {
        if (result == null) {
        ...
        if (functionRename != null && !safeTrim(functionRename.getOldName()).isEmpty() && !safeTrim(functionRename.getNewName()).isEmpty()) {
    ...
    private String buildApplyConfirmationMessage(RenameItem functionRename, List<RenameItem> variableRenames) {
