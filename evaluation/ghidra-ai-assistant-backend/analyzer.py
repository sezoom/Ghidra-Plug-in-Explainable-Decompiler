...
def analyze(task: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # ── feedback loop ────────────────────────────────────────────────────────
    source_json_path = payload.get("source_json_path")
    skip_reflection = payload.get("skip_reflection", False)

    if source_json_path:
        result = _correction_loop(
            component,
            request_payload,
            result,
            source_json_path,
            max_attempts=1 if skip_reflection else 3,
        )
...
def _correction_loop(
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        if report.false_rate == 0.0 or attempt == max_attempts:
            ...
            result["control_report"] = {  # ← new
                "false_rate": report.false_rate,
                "total_nb_of_tested": report.total_nb_of_tested,
                "control_attempts": attempt,
                "errors": {
                    "functions": report.list_function_false,
                    "variables": report.list_local_variables_false,
                    "calls": report.list_calls_false,
                },
            }
            return result
        print(
